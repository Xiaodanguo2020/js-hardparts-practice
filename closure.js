...
function after(count, func) {
  let counter = 0
  return function callFuncAfter(...args) {
    counter++
    if (counter >= count) {
      return func(...args)
    }
...
function saveOutput(func, magicWord) {
  const output = {}
  return function withSavedOutput(arg) {
    if (arg === magicWord) {
      return output
    }
...
function cycleIterator(array) {
  let currentIdex = 0
  return function (){
    currentIdex = currentIdex % array.length
    const currentElement = array[currentIdex]
    currentIdex++
...
function makeFuncTester(arrOfTests) {
  ...
  return function(callback){
    return arrOfTests.every(([input, output]) => output === callback(input))
  }
}
